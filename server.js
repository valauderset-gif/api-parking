...
for (const p of parkings) etat[p.id] = { libres: p.capacite, majLe: null }; // au départ : parking vide
...
function vue(p) {
  ...
  return {
    ...p,
    libres,
    occupes: libres == null || p.capacite == null ? null : p.capacite - libres,
    statut: libres == null ? "inconnu" : libres === 0 ? "complet" : p.capacite && libres < p.capacite * 0.1 ? "presque complet" : "disponible",
    majLe: e.majLe,
  };
}
...
// Mise à jour venant d'une source externe (connecteur open data)
function majExterne({ id, nom, capacite, lat, lon, libres, source }) {
  let p = parkings.find((x) => x.id === id);
  if (!p) { p = { id }; parkings.push(p); etat[id] = { libres: null, majLe: null }; }
  const avant = JSON.stringify(vue(p));
  Object.assign(p, { nom, capacite, lat, lon, source });
  etat[id].libres = libres;
  const apres = vue(p);
  if (JSON.stringify(apres) !== avant) { etat[id].majLe = new Date().toISOString(); diffuser(p); }
}

// Source des données : SOURCE=stgallen dans les variables Render pour la démo Saint-Gall
if (process.env.SOURCE === "stgallen") {
  parkings.length = 0; // on remplace les parkings de parkings.json par ceux de Saint-Gall
  require("./connecteur-stgallen").demarrer(majExterne);
}

// Page d'affichage à intégrer dans un site via <iframe>
app.get("/widget", (req, res) => res.sendFile(__dirname + "/widget.html"));
app.get("/carte", (req, res) => res.sendFile(__dirname + "/carte.html"));
